#!/usr/bin/env python3
"""
Multi-Registry Asset Manager Addon Packaging Script
===================================================

This script packages the Multi-Registry Asset Manager addon for distribution.
Creates a ZIP file that can be installed directly in Blender.
Supports multiple asset registries including Polyhaven with extensible architecture.
...
def discover_addon_files(exclude_patterns):
    ...
    for item in os.listdir('.'):
        if os.path.isfile(item):
            # Include essential addon files and all Python modules
            if item in ['__init__.py', 'blender_manifest.toml', 'addon_ai.json', 'README.md'] or item.endswith('.py'):
...
def create_addon_package(output_dir="dist", version=None):
    ...
    package_name = f"cr8_sets_v{version}.zip"
    ...
    with zipfile.ZipFile(package_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        for directory in addon_directories:
            if os.path.exists(directory) and os.path.isdir(directory):
                ...
                add_directory_to_zip(zf, directory, "cr8_sets", exclude_patterns)
            else:
    ...
    print(f"4. Enable 'Multi-Registry Asset Manager' in the addon list")
    print(f"5. Use with B.L.A.Z.E or other AI systems for natural language asset search")
    print(f"6. Supports Polyhaven assets (HDRIs, textures, models) with extensible registry system")
...
def create_development_package():
    ...
    package_name = "multi_registry_asset_manager_dev.zip"
    package_path = output_path / package_name
    ...
    with zipfile.ZipFile(package_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        for file_name in addon_files:
            if os.path.exists(file_name):
                arcname = f"multi_registry_asset_manager/{file_name}"
                zf.write(file_name, arcname)
                print(f"  Added: {file_name}")
        ...
        for directory in addon_directories:
            if os.path.exists(directory) and os.path.isdir(directory):
                print(f"  Adding directory: {directory}/")
                add_directory_to_zip(zf, directory, "multi_registry_asset_manager", exclude_patterns)
...
def validate_addon_structure():
    ...
    if not addon_directories:
        ...
        print("   Expected directories with .py files (e.g., handlers/, registries/, integration/)")
        return False
...
def show_addon_info():
    ...
    print("🎯 Multi-Registry Asset Manager")
    print("=" * 50)
    print("AI-powered Blender addon for asset management across multiple registries")
    print()
    print("📋 Features:")
    print("  • Polyhaven integration (HDRIs, textures, models)")
    print("  • Natural language search with AI scoring")
    print("  • Automatic scene integration and material setup")
    print("  • Extensible registry architecture")
    print("  • B.L.A.Z.E compatible for conversational asset management")
    print("  • No authentication required for Polyhaven")
    print()
    print("🔧 Current Registries:")
    print("  • Polyhaven (active - refactored into modular architecture)")
    print("  • BlenderKit (planned - pending API fixes)")
    print()
    print("📦 Packaging:")
    ...
    print("  • Supports nested directory structures (e.g., registries/polyhaven/)")
    print()


def main():
    parser = argparse.ArgumentParser(
        description="Package Multi-Registry Asset Manager addon (with dynamic file discovery)")
    parser.add_argument(
    ...
    print("🚀 Multi-Registry Asset Manager Packager")
    print("=" * 50)
    ...
    print("  2. Use with B.L.A.Z.E for conversational asset management")
    print("  3. Test Polyhaven asset search and download functionality")
    print()
    print("💡 Pro tip: Add new registries or modules without updating this script!")
