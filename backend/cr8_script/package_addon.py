#!/usr/bin/env python3
"""
Blaze Script Addon Packaging Script
===================================

This script packages the Blaze Script addon for distribution.
Creates a ZIP file that can be installed directly in Blender.
Provides Python execution capabilities for AI agents.
"""
...
# Directory name inside the ZIP. Must match `id` in blender_manifest.toml —
# the router resolves handlers by matching the installed directory name against
# Blender's enabled-addon keys (bl_ext.user_default.cr8_script).
ADDON_DIR = "cr8_script"
...
def create_addon_package(output_dir="dist", version=None):
    ...
    package_name = f"{ADDON_DIR}_v{version}.zip"
    ...
    with zipfile.ZipFile(package_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        for directory in addon_directories:
            if os.path.exists(directory) and os.path.isdir(directory):
                ...
                add_directory_to_zip(zf, directory, ADDON_DIR, exclude_files)
            else:
    ...
    print(f"4. Enable 'Blaze Script' in the addon list")
    print(f"5. Ensure Blender AI Router (cr8-router) is also installed as prerequisite")
    print(f"6. The Script addon will provide Python execution capabilities to AI agents")
...
def show_addon_info():
    ...
    print("🎯 Blaze Script")
    print("=" * 40)
    print("Python execution capabilities for AI agents")
    print()
    print("📋 Features:")
    print("  • execute_python — run bpy code authored by B.L.A.Z.E")
    print("  • Captured stdout/stderr and full tracebacks returned to the agent")
    print("  • Deferred responses for renders and bakes (check_is_finished)")
    print("  • Weak sandbox blocking session-ending operators")
    print("  • Kill switch via CR8_ALLOW_CODE_EXEC=0")
    print()
...
def main():
    parser = argparse.ArgumentParser(
        description="Package Blaze Script addon")
    parser.add_argument(
    ...
    print("  4. Ask B.L.A.Z.E to do something no dedicated tool covers")
